...
if args.msOutput:
    if args.msOutput[-3:] == ".gz": msOutput = gzip.open(args.msOutput, "rt")
    else: msOutput = open(args.msOutput, "rt")
else: msOutput = sys.stdin

if args.outTrees:
    if args.outTrees[-3:] == ".gz": outTrees = gzip.open(args.outTrees, "wt")
    else: outTrees = open(args.outTrees, "wt")
else: outTrees = sys.stdout

if args.outData:
    if args.outData[-3:] == ".gz": outData = gzip.open(args.outData, "wt")
    else: outData = open(args.outData, "wt")
...
if args.outData:
    ...
    #write output
    outData.write("chrom\tstart\tend\n")
    for x in range(len(ends)): outData.write("1\t" + str(starts[x]) + "\t" + str(ends[x]) + "\n")
